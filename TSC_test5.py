...
df = df[df["ID"].isin(["E_DOLLB-1", "T_SUTBS-1"])]
...
# ITS BECAUSE THERE's TWO TIME SERIES PER UNIT TYPE AT THE MOMENT (ie the test data I've used has 2 BESS units and 2 solar units, but there's no distinction between them atm, so there's two separate time series of length 64_536 listed as the same one)
...
# adding BMU ID as a separate column gives the error: TypeError: classifiers do not support categorical features in exogeneous X. - think this needs to be an integer rather than a string
# ^^^^ okay this doesn't work either. You get this error now ValueError: cannot reshape array of size 516288 into shape (2,64536,2)


# THE MODEL WORKS WITH ONLY ONE TIME SERIES PER FUEL TYPE
# I WILL PROBABLY NEED TO TAKE AN AVERAGE OF THE REFERENCE PNs TO USE AS THE MODEL CLASSIFIER UNLESS THERE's A WAY TO HAVE MULTIPLE TIME SERIES IN THE MODEL

# To do this, I could take an average profile of the PN values divided by the max(abs(FPN)) during a day for each fuel type (eg if the max abs FPN on a day was 50MW, all FPNs would be a % of that)
