        ...
        def REMIT(self, start_date: str, end_date: str, only_last_revision: bool = True):
            ...
            def Remove_Overlaps(REMIT, start_date, end_date):
                ...
                for i, j in enumerate(IDs):
                    ...
                    temp = REMIT[REMIT["BMU ID"] == j].sort_values(by = ["SP_s", "SP_e", "Created time"])
                    
                    # removes data where there's overlapping entries, keeps most recently created ones
                    temp = temp.drop_duplicates(subset = ["BMU ID", "SP_s", "SP_e", "Unavailable MW"], keep = "last")
                    '''
                    ...
                    raise TypeError("It looks like this is successfully recognising the overlaps coded but I need to check for other types of overlaps which have yet to be coded")
                    
                    if j == "T_FOYE-1":
                        print(temp[["Created time", "SP_s", "SP_e", "Overlap"]])
                        sys.exit()
                    
                    if (len(temp[temp["Overlap"] != 0]) > 0) and (j not in x):
