...
class Data_Import:
    ...
    def __init__(self):
        ...
        # rename column names to match those on the server
        self.server_renames = {"Published time": "Forecast_created", 
                               "Start time": "Forecast_time",
                               "Date": "Forecast_date",
                               "Transmission demand": "MW"
                               }
    
        # order of columns on server to keep them consistent
        self.server_order = ["Model", "Forecast_created", "Forecast_time", "Forecast_date", "SP",
                             "Demand_type", "MW"]
    ...
    class Elexon_query():
        ...
        def Demand_data(self, start_date: str, end_date: str):
            ...
            df = df[(df["Date"] >= datetime.strptime(start_date, "%Y-%m-%d")) & (df["Date"] < datetime.strptime(end_date, "%Y-%m-%d"))]
            df = df.sort_values(by = "Start time").reset_index(drop = True)
        ...
        def DA_Demand_Forecast_data(self, start_date: str):
            ...
            del df["boundary"], df["National demand"]
            
            df["Model"] = "NESO"
            df["Demand_type"] = "ITSDO"
            
            df = df.rename(columns = Data_Import().server_renames)
            
            df = df[[i for i in Data_Import().server_order if i in df.columns.tolist()]]
            
            return df
        
        def DA_Renewable_Forecast_data(self, start_date: str):
        ...
        def Actual_Renewable_data(self, start_date: str):
            ...
            df = Data_Import().Elexon_query().Get_Data(ID = "/generation/actual/per-type/wind-and-solar", 
                                                       start_date = start_date,
                                                       end_date = end_date)
            
            df = df.rename(columns = Data_Import().column_renames)
            df = df.rename(columns = {"quantity": "MW"}) # haven't included in main rename dict as other datasets have a 'quantity' column which aren't MW
            ...
            # below removes the onshore/offshore component of wind, and treats it as just wind
            df["Fuel type"] = df["businessType"].str.replace(" generation", "")
            df = df.groupby(["Start time", "Date", "SP", "Fuel type"])["MW"].sum().reset_index()
